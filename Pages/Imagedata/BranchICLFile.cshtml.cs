...
{
    public class BranchICLFileModel : PageModel
    {
        ...
        public List<Response_OECFile_List> reports { get; set; }
        ...
        public JsonResult OnPostGetFiles([FromBody] Params_Rep PR)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    NIBC_ICL_DownloadRequest req = new NIBC_ICL_DownloadRequest
                    {
                        DDL_Cycle = PR.DDL_Cycle,
                        dtFrom = dtFrom,
                        dtTo = dtTo,
                        sUserId = sUserId,
                        DDL_City = (PR.DDL_City != "All" ? PR.DDL_City : ""),
                        DDL_Bank = PR.DDL_Bank,
                        PageName = "ImageInwardOECFile"
                    };

                    var postTask = client.PostAsJsonAsync<NIBC_ICL_DownloadRequest>("BranchICL_File", req);
                    postTask.Wait();

                    var result = postTask.Result;
                    if (result.IsSuccessStatusCode)
                    {
                        var readTask = result.Content.ReadAsAsync<Response_OECFile_2>();
                        readTask.Wait();

                        if (readTask.Result.responseMessage.ToLower() == "success")
                            reports = readTask.Result.lstOEC;
        ...
        public ActionResult OnPostDownloadFile(string? v301x913, string? c036x9210)
        {
            ...
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var responseTask = client.PostAsJsonAsync<NIBC_ICL_Download_ImgRequest>("BranchICL_File_Download", req);
                    responseTask.Wait();
