...
{
    public class RtnDtLogRptModel : PageModel
    {
        ...
        public void OnGet()
        {
            try
            {
                ...
                apiURL = MyConfig.GetValue<string>("AppSettings:ibcs.api") + "RtnDtLogRpt/";
        ...
        public ActionResult OnPostDownloadFile(string? v301x913, string? c036x9210)
        {
            try
            {
                ...
                if (HttpContext.Session.GetString(SessionFile) != null)
                {
                    ...
                    {
                        if (HttpContext.Session.GetString(RtnDtLogRptModel.SessionCount) != null)
                        {
                            t_ = Convert.ToInt32(HttpContext.Session.GetString(RtnDtLogRptModel.SessionCount));
                        }
                    ...
                    else
                    {
                        ...
                        HttpContext.Session.SetString(RtnDtLogRptModel.SessionCount, "0");
                    }
                }
                else
                {
                ...
                if (repsave != null)
                {
                    ...
                    if (chkr == 1)
                    {
                        Response.Redirect("/Reports/RtnDtLogRpt");
                    }
                }
                ...
                using (var client = new HttpClient())
                {
                    ...
                    string ct = HttpContext.Session.GetString(RtnDtLogRptModel.SessionCount);
        ...
        public JsonResult OnPostGetFiles([FromBody] Params_Rep PR)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(apiURL);
                    //HTTP GET
                    var responseTask = client.GetAsync("getBrReport");
                    responseTask.Wait();
