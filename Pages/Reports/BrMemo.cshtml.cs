...
{
    public class BrMemoModel : PageModel
    {
        public const string SessionCount = "_count";
        public const string SessionFile = "_file";
        public static string apiURL = "", comm_apiURL="", cpuapiURL="";
        ...
        public void OnGet()
        {
            try
            {
                ...
                apiURL = MyConfig.GetValue<string>("AppSettings:ibcs.api") + "BrMemo/";
                cpuapiURL = MyConfig.GetValue<string>("AppSettings:ibcs.api") + "CPUMemo/";
        ...
        private void BindCycleDD()
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(cpuapiURL);

                    var postTask = client.GetAsync("GetCycleListReturn");
                    postTask.Wait();
        ...
        public ActionResult OnPostDownloadFile(string? v301x913, string? c036x9210)
        {
            try {
                ...
                if (HttpContext.Session.GetString(SessionFile) != null)
                {
                    ...
                    {
                        if (HttpContext.Session.GetString(BrMemoModel.SessionCount) != null)
                        {
                            t_ = Convert.ToInt32(HttpContext.Session.GetString(BrMemoModel.SessionCount));
                        }
                    ...
                    else
                    {
                        ...
                        HttpContext.Session.SetString(BrMemoModel.SessionCount, "0");
                    }
                }
                else
                {
                ...
                if (repsave != null)
                {
                    ...
                    if (chkr == 1)
                    {
                        Response.Redirect("/Reports/BrMemo");
                    }
                }
            ...
            byte[] MyData = new byte[0];
                ...
                using (var client = new HttpClient())
                {
                    ...
                    string ct = HttpContext.Session.GetString(BrMemoModel.SessionCount);
